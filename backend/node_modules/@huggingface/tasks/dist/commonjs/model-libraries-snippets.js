...
const transformers = (model) => {
    ...
    if (info.processor) {
        ...
        autoSnippet.push("# Load model directly", `from transformers import ${info.auto_model}`, `model = ${info.auto_model}.from_pretrained("${model.id}"` +
            remote_code_snippet +
    ...
    if (model.pipeline_tag && library_to_tasks_js_1.LIBRARY_TASK_MAPPING.transformers?.includes(model.pipeline_tag)) {
        ...
        pipelineSnippet.push("from transformers import pipeline", "", `pipe = pipeline("${model.pipeline_tag}", model="${model.id}"` + remote_code_snippet + ', device_map="auto")');
        if (model.tags.includes("conversational")) {
            if (model.tags.includes("image-text-to-text")) {
